...
public class Model extends Renderable implements RSModel {
    ...
    private Model(int modelId) {
        ...
        if (is[is.length - 1] == -1 && is[is.length - 2] == -1) {
            decode622(is,modelId);
        } else {
            decode_old(is,modelId);
        }
    ...
    public void decode622(byte[] data, int modelId) {
        try {
            if (data[data.length - 1] == -1 && data[data.length - 2] == -1) {
                ...
                ModelHeader footer = new ModelHeader();
                aClass21Array1661[modelId] = footer;
                footer.data = data;
                footer.vertices = numVertices;
                footer.faces = trianglesCount;
                footer.texture_faces = numberOfTexturesFaces;
                ...
                int version = 0;
                if (hasVersion) {
                    first.currentPosition -= 7;
                    version = first.readUnsignedByte();
                    first.currentPosition += 6;
                }
                ...
                int modelVerticesX = first.readUShort();
                int modelVerticesY = first.readUShort();
                int modelVerticesZ = first.readUShort();
                int faceIndices = first.readUShort();
                int textureIndices = first.readUShort();
                int numVertexSkins;
                if (hasExtendedVertexSkins) {
                    numVertexSkins = first.readUShort();
                ...
                if (hasExtendedTriangleSkins) {
                    numFaceSkins = first.readUShort();
                } else if (hasFaceSkins == 1) {
                    numFaceSkins = trianglesCount;
                } else {
                    numFaceSkins = 0;
                }
                ...
                if (numberOfTexturesFaces > 0) {
                    textureTypes = new byte[numberOfTexturesFaces];
                    first.currentPosition = 0;
                    for (int tri = 0; tri < numberOfTexturesFaces; tri++) {
                        byte type = textureTypes[tri] = first.readSignedByte();
                        if (type == 0) {
                ...
                face_render_priorities = new byte[trianglesCount];
                if (modelPriority != 255) {
                    face_priority = (byte) modelPriority;
                }
                if (hasFaceAlpha == 1) {
                    face_alpha = new byte[trianglesCount];
                }
                ...
                if (hasFaceTextures == 1 && numberOfTexturesFaces > 0) {
                    textures = new byte[trianglesCount];
                }
                triangleColours = new short[trianglesCount];
                if (numberOfTexturesFaces > 0) {
                ...
                first.currentPosition = vertexFlagsOffset;
                second.currentPosition = vertexXOffsetPosition;
                third.currentPosition = vertexYOffsetPosition;
                fourth.currentPosition = vertexZOffsetPosition;
                fifth.currentPosition = vertexSkinsOffset;
                int baseX = 0;
                int baseY = 0;
                int baseZ = 0;
                for (int vertex = 0; vertex < numVertices; vertex++) {
                    ...
                    if (hasVertexSkins == 1) {
                        if (hasExtendedVertexSkins) {
                            vertexVSkin[vertex] = fifth.getSmartNS();
                        } else {
                            vertexVSkin[vertex] = fifth.readUnsignedByte();
                        }
                    }
                }
                first.currentPosition = faceColorsOffset;
                second.currentPosition = faceTypesOffset;
                third.currentPosition = facePrioritiesOffset;
                fourth.currentPosition = faceAlphasOffset;
                fifth.currentPosition = faceSkinsOffset;
                sixth.currentPosition = faceMaterialsOffset;
                seventh.currentPosition = faceTextureIndicesOffset;
                for (int face = 0; face < trianglesCount; face++) {
                    triangleColours[face] = (short) first.readUShort();
                    ...
                    if (modelPriority == 255) {
                        ...
                        if (modelId == 64488 || modelId == 60253 || modelId == 52576 || modelId == 62022) { // priority fix through code
                            face_render_priorities[face] = 10;
                        }
                    }
                    if (hasFaceAlpha == 1) {
                        face_alpha[face] = fourth.readSignedByte();
                    ...
                    if (hasFaceSkins == 1) {
                        if (hasExtendedTriangleSkins) {
                            triangleTSkin[face] = fifth.getSmartNS();
                        } else {
                            triangleTSkin[face] = fifth.readUnsignedByte();
                        }
                    }
                    if (hasFaceTextures == 1) {
                        materials[face] = (short) (sixth.readUShort() - 1);
                        if (materials[face] == 39 || materials[face] > 50) {
                            materials[face] = -1;
                        }
                    }
                    if (textures != null) {
                        if (materials[face] != -1) {
                            ...
                            textures[face] = (byte) -1;
                        }
                    }
                }
                first.currentPosition = faceIndicesOffset;
                second.currentPosition = facesCompressTypeOffset;
                short a = 0;
                short b = 0;
                short c = 0;
                int acc = 0;
                for (int face = 0; face < trianglesCount; face++) {
                    ...
                    if (type == 1) {
                        a = (short) (first.readSmart() + acc);
                        acc = a;
                        b = (short) (first.readSmart() + acc);
                        acc = b;
                        c = (short) (first.readSmart() + acc);
                        acc = c;
                        facePointA[face] = a;
                        facePointB[face] = b;
                        facePointC[face] = c;
                ...
                first.currentPosition = simpleTexturesOffset;
                second.currentPosition = complexTexturesOffset;
                third.currentPosition = texturesScaleOffset;
                fourth.currentPosition = texturesRotationOffset;
                fifth.currentPosition = texturesDirectionOffset;
                sixth.currentPosition = texturesTranslationOffset;
                for (int index = 0; index < numberOfTexturesFaces; index++) {
                    int type = textureTypes[index] & 0xff;
                    if (type == 0) {
                        textures_face_a[index] = (short) first.readUShort();
                        textures_face_b[index] = (short) first.readUShort();
                        textures_face_c[index] = (short) first.readUShort();
                    }
                    if (type == 1) {
                        textures_face_a[index] = (short) second.readUShort();
                        textures_face_b[index] = (short) second.readUShort();
                        textures_face_c[index] = (short) second.readUShort();
                        if (version < 15) {
                            third.readUShort();
                            if (version < 14) {
                                third.readUShort();
                        ...
                        fourth.readSignedByte();
                        fifth.readSignedByte();
                        sixth.readSignedByte();
                    }
                    if (type == 2) {
                        textures_face_a[index] = (short) second.readUShort();
                        textures_face_b[index] = (short) second.readUShort();
                ...
                first.currentPosition = particleEffectsOffset;
                ...
                if (modelPriority != 255) {
                    for (int face = 0; face < trianglesCount; face++) {
                        face_render_priorities[face] = (byte) modelPriority;
                    }
                }
                ...
                offset += numberOfTexturesFaces * 6;
                int vertexXOffsetPosition = offset;
                offset += modelVerticesX;
                int vertexYOffsetPosition = offset;
                offset += modelVerticesY;
                ...
                for (int face = 0; face < trianglesCount; face++) {
                    ...
                    if (hasFaceTypes == 1) {
                        ...
                        if ((flag & 0x2) == 2) {
                            textures[face] = (byte) (flag >> 2);
                            materials[face] = (short) triangleColours[face];
                            triangleColours[face] = (short) 127;
                            if (materials[face] != -1) {
                                usesMaterials = true;
                            }
                        } else {
                            textures[face] = (byte) -1;
                            materials[face] = (short) -1;
                        }
                    }
                    if (modelPriority == 255) {
                        face_render_priorities[face] = third.readSignedByte();
                ...
                first.currentPosition = faceMappingsOffset;
                for (int face = 0; face < numberOfTexturesFaces; face++) {
                    textureTypes[face] = (byte) 0;
                    textures_face_a[face] = (short) first.readUShort();
                    textures_face_b[face] = (short) first.readUShort();
                    textures_face_c[face] = (short) first.readUShort();
                }
                if (textures != null) {
                    ...
                    for (int face = 0; face < trianglesCount; face++) {
                        int texture = textures[face] & 0xff;
                        if (texture != 255) {
                            if ((textures_face_a[texture] & 0xffff) == facePointA[face]
                                    && (textures_face_b[texture] & 0xffff) == facePointB[face]
                                    && (textures_face_c[texture] & 0xffff) == facePointC[face]) {
                                textures[face] = (byte) -1;
                            } else {
                                usesMapping = true;
                            }
            ...
            System.out.println("Model " + modelId + " Is Causing Issues Check the Array");
        }
